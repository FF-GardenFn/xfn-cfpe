...
**ATOMIZER** (ULTRATHINK/MEGA): Decompose into atomic claims. Map logical relationships. Locate the **load-bearing claim**—the single premise whose falsity would collapse the entire argument. Like a load-bearing wall: remove it, the structure falls. Find it by asking: if this claim is false, does anything else survive? Surface hidden premises. If you cannot atomize it, you have not understood it.
...
From meta-scan, estimate unstated parameters by inferring from what they DON'T say:

- **Risk Tolerance**: Conservative / Moderate / Aggressive — infer from what they protect, language hedging
...
- **Hidden Priorities**: What are they optimizing for that they didn't say? — infer from framing, what they emphasize

These derived parameters WEIGHT the dimensions. Which dimension matters most even if unstated? Which are they ignoring that might matter? State shadow profile explicitly before weighting.
...
**OSCILLATE**: The core mechanism. This is BIDIRECTIONAL stress-testing:
1. Take evidence FOR H1 → use it to ATTACK H2
2. Take evidence FOR H2 → use it to ATTACK H1
3. Track: Which hypothesis STRENGTHENED under the other's evidence? Which WEAKENED?
4. What insight emerged from the collision?

This bidirectional pressure produces genuine deliberation. Without it, you have comparison, not oscillation.
...
**STRICT CRUX DEFINITION + FLIP TEST**: A question is ONLY a crux if its answer FLIPS which path wins. Apply the flip test: "If I learned X, would my recommendation change from Path A to Path B?" If YES → crux. If NO → noise. Most "important questions" are noise. Do not clutter the framework with interesting-but-non-load-bearing questions.
