...
"""
================================================================================
RL-O-CoV V5: Resonance-shaped RL for Oscillatory Chain of Verification
================================================================================

V5 fixes the V4 post-launch review findings (2026-07-30):
- Phase boundaries match marker FORMS ([X] / **X** / line-start "X:" / heading),
  so prose words like "answer" no longer truncate sections or fail structure.
- Final-answer extraction takes the LAST "Answer:" line, not the first.
- evaluate() no longer reseeds RNG at all: greedy decode consumes no
  randomness, and V4's fork_rng(devices=[]) + manual_seed silently reset the
  CUDA sampling stream on every eval, correlating exploration across segments.
- Sampling temperature is a config field; history rows stay aligned (NaN fill).
- Synthetic hard pool doubled to 20 hand-verified problems (V4: 10 x 20 copies).
- Pure-text parsing lives at module level with a stdlib self-test (no torch).

V4 remains the archived Colab-run artifact and is unchanged; V5 has not been run.
================================================================================
"""
...
class TrainingConfigV5:
    """V5 config: stable RL estimator plus frozen mechanistic reward judge."""
    ...
    sample_temperature: float = 0.7  # V5: was hardcoded in generate()
    ...
    # OUTPUT
    output_dir: str = "./rl_ocov_v5_checkpoints"
...
def normalize_answer_text(answer: Any) -> str:
    ...
    # V5: "is" may be followed by ":" or "=" ("The answer is: 42"), and the
    # trailing punctuation must come off before the "$" wrapper ("$42$.").
    text = re.sub(r"(?i)^(?:the\s+)?(?:final\s+)?answer\s*(?:is\s*[:=]?|[:=])\s*", "", text)
...
def extract_final_answer(text: str) -> str:
    ...
    V5: takes the LAST "Answer:" line. V4 used re.search (first match), so an
    early mid-reasoning "Answer:" line shadowed the final one the template
    mandates ("End with exactly: Answer: <final answer>").
    """
    if not text:
        return ""
    ...
    if answer_matches:
        return normalize_answer_text(answer_matches[-1].group(1))
...
# %%
# ============================================================================
# SECTION PARSING (pure text, no torch — stdlib-testable)
# ============================================================================
# V5: moved out of OCovRewardCalculator and reworked. V4 located phase
# boundaries with bare-substring search on uppercased text, so the WORD
# "answer" (or "oscillation") anywhere in prose acted as a section boundary:
# "[SYNTHESIZE] The answer is 42" truncated the synthesize section to 4 chars
# and scored the phase missing. V5 only recognizes deliberate marker FORMS:
#   [NAME] or **NAME**  anywhere (typographically deliberate),
#   "NAME:" or "# NAME"  at line start only (the forms prose collides with).

def _phase_marker_pattern(name: str) -> str:
    n = re.escape(name)
    return (
        rf"\[\s*{n}\s*\]"                  # [NAME]
        rf"|\*\*\s*{n}\s*\*\*"             # **NAME**
        rf"|(?:^|\n)[ \t]*#{{1,6}}[ \t]+{n}\b"  # markdown heading at line start
        rf"|(?:^|\n)[ \t]*{n}[ \t]*:"      # NAME: at line start
    )


def find_phase_marker(text: str, name: str, start: int = 0) -> Optional[re.Match]:
    return re.compile(_phase_marker_pattern(name), re.IGNORECASE).search(text, start)


def phase_marker_present(text: str, name: str) -> bool:
    return find_phase_marker(text, name) is not None


def extract_sections(text: str) -> Dict[str, str]:
    """Split a response into DIALECTICA phase sections by marker forms."""
    ...
    for section_name, markers, next_markers in phase_specs:
        ...
        for marker in markers:
            m = find_phase_marker(text, marker)
            if m is not None and (best is None or m.start() < best.start()):
                best = m
        if best is None:
            continue
        content_start = best.end()
        end = len(text)
        for next_marker in next_markers:
            nm = find_phase_marker(text, next_marker, content_start)
            if nm is not None:
                end = min(end, nm.start())
        # A ":" may trail the bracketed/bold marker forms; strip it with the whitespace.
        sections[section_name] = text[content_start:end].strip().lstrip(":").strip()

    return sections


def extract_hypotheses(hypothesis_section: str) -> Tuple[str, str]:
...
def generate_synthetic_hard_problems(n: int, exclude_questions: Optional[set] = None) -> List[Dict]:
    ...
    V5: pool doubled to 20 hand-verified problems (every label re-derived by
    hand before inclusion — wrong labels were V3's cardinal sin). With
    benchmark data gated off (the default), the hard mix is filled by cycling
    this pool: at the default hard target of 200 that is 10 copies of each
    (V4 cycled 10 problems 20x). The repetition is a deliberate
    anti-contamination tradeoff; provide local MATH-500 data and set
    allow_benchmark_hard_data=True for genuinely diverse hard problems.
    """
    excluded = exclude_questions or set()
    ...
    problems = [
        ...
        # V5 additions — each label hand-derived at authoring time:
        {
        ...
        },
        {
            ...
            "question": "What is the sum of the digits of 2^15?",
            "answer": "26",
            "difficulty": 2
        },
        {
            # n = 2+7k <= 100 -> k = 0..14 -> 15 values
            "question": "How many positive integers n at most 100 satisfy n ≡ 2 (mod 7)?",
            "answer": "15",
            "difficulty": 2
        },
        {
        ...
        {
            # ord(3 mod 7) = 6: 3,2,6,4,5,1; 100 mod 6 = 4 -> 3^4 = 81 = 4 (mod 7)
            "question": "Compute the remainder when 3^100 is divided by 7.",
            "answer": "4",
            "difficulty": 2
        },
        {
            # n(n-3)/2 = 12*9/2 = 54
            "question": "How many diagonals does a convex polygon with 12 sides have?",
            "answer": "54",
            "difficulty": 2
        },
        {
            # 24 + 26 + 28 = 78 -> largest 28
            "question": "If the sum of three consecutive even integers is 78, what is the largest of them?",
            "answer": "28",
            "difficulty": 1
        },
        {
            # lcm(3,4,5) = 60 -> 60 + 1 = 61
            "question": "Find the smallest positive integer greater than 1 that leaves remainder 1 when divided by each of 3, 4, and 5.",
            "answer": "61",
            "difficulty": 2
        },
        {
            # circular permutations: (8-1)! = 5040
            "question": "In how many ways can 8 people be seated around a round table, if rotations are considered the same?",
            "answer": "5040",
            "difficulty": 2
        },
        {
            # 1/7 = 0.(142857), period 6; 2026 mod 6 = 4 -> 4th digit = 8
            "question": "What is the 2026th digit after the decimal point in the decimal expansion of 1/7?",
            "answer": "8",
            "difficulty": 3
        },
    ]
...
class OCovRewardCalculator:
    ...
    def __init__(self, model, tokenizer, config: TrainingConfigV5):
        self.model = model
    ...
    # V5: parsing lives at module level (stdlib-testable without torch);
    # these delegates keep the calculator's API unchanged.
    def _phase_marker_present(self, text: str, phase: str) -> bool:
        return phase_marker_present(text, phase)

    def extract_sections(self, text: str) -> Dict[str, str]:
        return extract_sections(text)

    def extract_hypotheses(self, hypothesis_section: str) -> Tuple[str, str]:
        return extract_hypotheses(hypothesis_section)
...
class RLOCovTrainerV5:
    ...
    def generate(self, prompt: str, do_sample: bool = True) -> Tuple[str, torch.Tensor, int]:
        ...
        if do_sample:
            generation_kwargs["temperature"] = self.config.sample_temperature
    ...
    def _apply_missing_phase_penalty(self, reward: float, metrics: Dict) -> Tuple[float, int]:
        """Flat per-missing-phase penalty on top of the fractional structure score.

        Intentional double counting (reviewed 2026-07-30, kept by design):
        check_structure already scales its reward by the fraction of phases
        present; this adds a further flat cost per missing phase so that
        dropping a phase always hurts more than the fraction alone implies.
        Correctness still dominates: a fully-penalized correct answer outscores
        any wrong answer (see the wrong_answer_max_reward cap).
        """
    ...
    def train_batch(self, batch_items: List[Dict]) -> Dict:
        ...
        # V5: always append so every history list stays step-aligned (V4
        # skipped steps with no resonance-eligible samples, desynchronizing
        # this list from history["step"]).
        self.history["similarity"].append(
    ...
    def evaluate(self, eval_data: List[Dict], num_samples: int = None) -> Dict:
        ...
        # V5: no RNG reseeding here. Greedy decode consumes no randomness, and
        # V4's fork_rng(devices=[]) + manual_seed restored only the CPU state
        # while silently resetting the CUDA sampling stream on every eval,
        # correlating training exploration across inter-eval segments.
        #
        # use_centers=False keeps eval independent of training-time center
        # state, so checkpoint evals stay comparable to each other. The cost:
        # eval goldilocks_rate is computed on UNcentered similarities and is
        # not directly comparable to the train-time (centered) rate. Compare
        # eval-to-eval only.
        try:
...
def main():
    ...
    print("=" * 70)
    print("RL-O-CoV V5: FROZEN CENTERED RESONANCE RL")
    print("   V5 fixes: marker-form section parsing, last-answer extraction,")
    print("   eval RNG isolation, aligned history, config temperature, 20-problem pool")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    # Create trainer
    trainer = RLOCovTrainerV5(model, tokenizer, config)
